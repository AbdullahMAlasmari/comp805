...
def test_check_is_greater_than(results_dict):
    ...
    message = "----"
    try:
        result = lab1.check_is_greater_than(5, 2)
        if not isinstance(result, bool):
            message += 'Did not return a boolean - Fail'
            test_failed(results_dict)
        else:
            message += 'A boolean was returned - Pass'
            test_passed(results_dict)
            # now check if the result is true (5 is greater than 2)
            if result is True:
                message += '\n---- 5 is greater than 2, True - Pass'
                test_passed(results_dict)
            else:
                message += '\n---- 2 is greater than 5, False - Fail'
                test_failed(results_dict)
            # try again, with number1 > number2
            result = lab1.check_is_greater_than(2, 5)
            if result is False:
                message += '\n---- 4 is greater than 5, False - Pass'
                test_passed(results_dict)
            else:
                message += '\n---- 5 is less than 6, True - Fail'
                test_failed(results_dict)
    except AttributeError:
...
def test_check_if_number_equal5(results_dict):
    """
    Checks that lab1.give_me_an_integer() returns an integer type
    """
    print("")
    print("Testing lab1.check_if_number_equal5()")
    message = "----"
    try:
        result = lab1.check_if_number_equal5()
        if result != 5:
            message += 'Did not return 5 - Fail'
            test_failed(results_dict)
        else:
            message += 'the number is 5 - Pass'
            test_passed(results_dict)
    except AttributeError:
        message += 'Could not find lab1.check_if_number_equal5() - Fail'
        test_failed(results_dict)
    print(message)
...
def run():
    ...
    test_check_if_number_equal5(results_dict)
